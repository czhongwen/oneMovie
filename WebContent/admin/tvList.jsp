<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
	<%@page import="org.lanqiao.bean.TV" %>
...
<body>
	<form method="post" action="TvMangerServket" id="listform">
		<div class="panel admin-panel">
			...
			<div class="padding border-bottom">
				<ul class="search" style="padding-left: 10px;">
					...
					<li><a href="VideoMangerServlet?vedioType= moive&currentPage=1&flag=page"><button class="button bg-main  icon-check-square-o" type="button">电影</button></a> &nbsp;&nbsp; &nbsp;&nbsp;
					    <a href="VideoMangerServlet?vedioType= tv&currentPage=1&flag=page"><button class="button bg-main border-red icon-check-square-o" type="button">电视剧</button></a> &nbsp;&nbsp; &nbsp;&nbsp;
			...
			<table class="table table-hover text-center">
				...
				<c:forEach items="${requestScope.list }" var="tv">
					<tr id="tr${tv.tvId }">
						<td style="text-align: left; padding-left: 20px;">
							<input type="checkbox" id="id" name="id[]" value="${tv.tvId }"/>
						</td>
						<td>${tv.tvName }</td>
						<td width="10%"><img src="${tv.tvPic }" alt="" width="70" height="50" /></td>
						<td>${tv.tvComment }</td>
						<td><font color="#00CC99">${tv.tvPlay }</font></td>
						<td>${tv.tvScore }</td>
						<td>${tv.loadTime }</td>
						<td>
							<div class="button-group">
								<a href="javascript:void(0)" href="javascript:void(0)" onclick="excuteUpdate('${tv.tvId }')" class="button border-red " style="padding: 5px 15px;" > 修改</a>
								<a href="javascript:void(0)" href="javascript:void(0)" onclick="return del('${tv.tvId }')" class="button border-red " style="padding: 5px 15px;" > 删除</a>
							</div>
						</td>
					</tr>
				</c:forEach>
				...
				<tr>
					<td colspan="8">
			        	...
			        	<c:if test="${requestScope.page.currentPage>1 && requestScope.page.currentPage!=requestScope.page.totalPage   }">
			        		<a href="TvMangerServlet?currentPage=${requestScope.page.currentPage-1 }${requestScope.key }">上一页</a> 
			        		<a href="TvMangerServlet?currentPage=${requestScope.page.currentPage-1 }${requestScope.key }">${requestScope.page.currentPage-1 }</a>
			        	</c:if>
			        	
			        	<c:if test="${requestScope.page.currentPage>1 && requestScope.page.currentPage==requestScope.page.totalPage   }">
			        		<a href="TvMangerServlet?currentPage=${requestScope.page.currentPage-1 }${requestScope.key }">上一页</a>
			        		<a href="TvMangerServlet?currentPage=${requestScope.page.currentPage-2 }${requestScope.key }">${requestScope.page.currentPage-2 }</a> 
			        		<a href="TvMangerServlet?currentPage=${requestScope.page.currentPage-1 }${requestScope.key }">${requestScope.page.currentPage-1 }</a>
			        	</c:if>
			        	...
			        	<c:if test="${requestScope.page.currentPage<requestScope.page.totalPage && requestScope.page.currentPage!=1 }">
			        		<a href="TvMangerServlet?currentPage=${requestScope.page.currentPage+1 }${requestScope.key }">${requestScope.page.currentPage+1 }</a>
			        		<a href="TvMangerServlet?currentPage=${requestScope.page.currentPage+1 }${requestScope.key }">下一页</a>
			        	</c:if>
			        	...
			        	<a href="TvMangerServlet?currentPage=${requestScope.page.totalPage }${requestScope.key }">尾页</a>
						</div>
					</td>
				</tr>
